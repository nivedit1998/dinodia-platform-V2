...
export async function requireCustomer(request: Request): Promise<CustomerPrincipal> {
  ...
  const session = await prisma.customerSession.findUnique({ where: { id: verified.sessionId }, select: { id: true, customerAccountId: true, trustedDeviceId: true, revokedAt: true, expiresAt: true, securityVersion: true } });
  if (!trustedDevice || trustedDevice.customerAccountId !== verified.id || trustedDevice.revokedAt || !session || session.customerAccountId !== verified.id || session.trustedDeviceId !== trustedDevice.id || session.securityVersion !== trustedDevice.sessionVersion || session.revokedAt || session.expiresAt <= new Date()) fail(new Stage1AuthError(401, 'customer_session_invalid', 'The app session is no longer active'));
