...
const SUPABASE_PRIVACY_HARDENING_SQL = `-- Phase 7: Supabase visibility lockdown
...
export default function SupabasePrivacyHardeningClient({ installerName }: { installerName: string }) {
  return (
    <div className="min-h-screen bg-slate-50 px-4 py-10">
      <div className="mx-auto flex max-w-5xl flex-col gap-6">
        ...
        <div className="rounded-xl bg-white p-6 shadow-lg ring-1 ring-slate-200">
          <h1 className="text-2xl font-semibold text-slate-900">Supabase privacy hardening (SQL)</h1>
          <p className="mt-2 text-sm text-slate-600">
            Evidence page showing the SQL hardening script used to reduce default database visibility and introduce a
            break-glass audit trail.
          </p>
        </div>

        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="text-lg font-semibold text-slate-900">How to run</h2>
          <BulletList
            items={[
              'Run against staging first, then production.',
              'Run as a privileged DB user (owner/superuser) using DIRECT_URL.',
              'Create a ticket/change request for this operation and record approver.',
            ]}
          />
          <CodeBlock
            code={[
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="text-lg font-semibold text-slate-900">SQL script</h2>
          <p className="mt-2 text-sm text-slate-600">
            This mirrors the script used in the repo at <span className="font-mono">scripts/supabase_privacy_hardening.sql</span>.
            Keep this page updated if the script changes.
          </p>
          <CodeBlock code={SUPABASE_PRIVACY_HARDENING_SQL} />
        </section>
      </div>
    </div>
  );
