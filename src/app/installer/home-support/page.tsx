...
export default function HomeSupportPage() {
  ...
  const [issuedCodeExpiresAt, setIssuedCodeExpiresAt] = useState<string | null>(null);
  ...
  useEffect(() => {
    const clear = () => { setIssuedCode(null); setIssuedCodeExpiresAt(null); };
  ...
  useEffect(() => {
    if (!issuedCodeExpiresAt) return;
    const timer = window.setTimeout(() => { setIssuedCode(null); setIssuedCodeExpiresAt(null); }, Math.max(0, Date.parse(issuedCodeExpiresAt) - Date.now()));
    return () => window.clearTimeout(timer);
  }, [issuedCodeExpiresAt]);

  async function requestAccess(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setIssuedCode(null); setIssuedCodeExpiresAt(null); setError(""); setResult("");
  ...
  async function issueCode(ticket: Ticket, requestId: string) {
    setError(""); setResult(""); setIssuedCode(null); setIssuedCodeExpiresAt(null);
    ...
    if (!response.ok) { supportWindow?.close(); setError(body.error || "The support code could not be issued"); }
      ...
      setIssuedCode(String(body.code)); setIssuedCodeExpiresAt(String(body.codeExpiresAt));
      const deliver = () => {
  ...
  return <main className="mx-auto min-h-screen w-full max-w-5xl px-6 py-12">
    ...
    {issuedCode && <section className="mt-6 rounded-2xl border border-amber-300 bg-amber-50 p-5 text-amber-950" role="status"><p className="text-sm font-semibold">One-use support code</p><code className="mt-2 block select-all text-lg font-bold">{issuedCode}</code><p className="mt-2 text-sm">Expires {issuedCodeExpiresAt ? new Date(issuedCodeExpiresAt).toLocaleTimeString() : "soon"}. Do not copy this code into a ticket, URL or notes.</p></section>}
