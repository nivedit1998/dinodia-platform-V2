...
export default function InstallerPage() {
  ...
  async function assignWork(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/installer/workflows", { method: "POST", credentials: "same-origin", headers: { "Content-Type": "application/json", "Idempotency-Key": crypto.randomUUID() }, body: JSON.stringify({ attemptId, assignedEmployeeId, kind: "INITIAL_HUB_INSTALLATION", reason: workReason }) });
      const body = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(body.error || "Installation work could not be assigned");
      setActionMessage("Installation work assigned from the persisted hub attempt."); setAttemptId(""); setWorkReason("");
