...
export async function POST(request: Request) {
  try {
    ...
    const rawPairingCode = text(body.pairingCode ?? body.code, 160);
    ...
    if (!workflowId || !pairingCode || !idempotencyKey) throw new Stage1AuthError(400, 'provisioning_fields_invalid', 'Workflow, pairing code and idempotency key are required');
    await enforcePersistentRateLimit(`provisioning-approve:${employee.id}`, 20, 15 * 60 * 1000);
    const codeHash = sha256(pairingCode);
    const requestHash = sha256(JSON.stringify({ workflowId, codeHash }));
    const keyHash = sha256(`${employee.id}:${idempotencyKey}`);
    let result: { state: string; expiresAt: Date; homeId: string; hubInstallationId: string; serialNumber: string; claimPresentation: string | null; idempotentReplay: boolean } | undefined;
      ...
      try {
        result = await prisma.$transaction(async (tx) => {
          ...
          if (existing && existing.expiresAt > now && existing.responseDigest) {
            const replay = await tx.hubProvisioningAttempt.findFirst({ where: { codeHash }, select: { state: true, expiresAt: true, hubInstallationId: true, manufacturingIdentityId: true, companyWorkItemId: true, manufacturingIdentity: { select: { serialNumber: true } } } });
            ...
            return { state: replay.state, expiresAt: replay.expiresAt, homeId: replayWork.homeId, hubInstallationId: replay.hubInstallationId, serialNumber: replay.manufacturingIdentity.serialNumber, claimPresentation: null, idempotentReplay: true };
          }
          ...
          const attempt = await tx.hubProvisioningAttempt.findFirst({ where: { codeHash }, select: { id: true, manufacturingIdentityId: true, hubInstallationId: true, companyWorkItemId: true, codeHash: true, state: true, expiresAt: true, baseUrlPresentation: true } });
          ...
          if (attempt.state !== 'PRESENTED' && attempt.state !== 'EMPLOYEE_APPROVED' && attempt.state !== 'CHALLENGE_PENDING' && attempt.state !== 'CHALLENGE_ISSUED') throw new Stage1AuthError(409, 'pairing_state_invalid', 'The provisioning attempt is not ready for approval');
          ...
          const updated = await tx.hubProvisioningAttempt.update({ where: { id: attempt.id }, data: { companyWorkItemId: work.id, hubInstallationId, redeemerEmployeeId: employee.id, state: 'CHALLENGE_PENDING', updatedAt: now }, select: { state: true, expiresAt: true } });
          ...
          const stableResponse = { state: updated.state, expiresAt: updated.expiresAt, homeId, hubInstallationId, serialNumber: identity.serialNumber };
