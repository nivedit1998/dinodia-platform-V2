...
export async function POST(request: Request, context: { params: Promise<{ homeId: string }> }) {
  try {
    ...
    const handoff = await prisma.$transaction(async (tx) => {
      ...
      const hub = await tx.hubInstallation.findUnique({ where: { id: work.hubInstallationId }, select: { id: true, homeId: true, baseUrl: true, cloudUrl: true, manufacturingIdentity: { select: { encryptionPublicKey: true } } } });
