...
export async function POST(request: Request) {
  try {
    ...
    const verification = await prisma.cloudUrlVerification.create({ data: { hubInstallationId: hub.installation.id, homeId: hub.installation.homeId, reservedHostname: hostname, tunnelId, tunnelName, cloudUrl, challengeHash: sha256(challenge), status: 'PENDING', expiresAt: new Date(Date.now() + 5 * 60 * 1000) }, select: { id: true, expiresAt: true } });
    ...
    const signed = await remote.json().catch(() => ({})) as Record<string, unknown>;
    const responseBody = { version: 1, serial: signed.serial, cloudUrl: signed.cloudUrl, challenge: signed.challenge, tunnelId: signed.tunnelId, tunnelName: signed.tunnelName, timestamp: Number(signed.timestamp), identityFingerprint: signed.identityFingerprint, identityGeneration: Number(signed.identityGeneration), bodyHash: signed.bodyHash };
    const responseCanonical = JSON.stringify(responseBody);
    const unsignedBodyHash = sha256(JSON.stringify({ ...responseBody, bodyHash: undefined }));
    const signatureValid = remote.ok && signed.ok === true && signed.serial === hub.installation.serialNumberSnapshot && signed.cloudUrl === cloudUrl && signed.challenge === challenge && signed.tunnelId === tunnelId && signed.tunnelName === tunnelName && Number.isSafeInteger(responseBody.timestamp) && Math.abs(Date.now() - responseBody.timestamp) <= 5 * 60 * 1000 && signed.bodyHash === unsignedBodyHash && signed.identityFingerprint === hub.identity.signingKeyFingerprint && Number(signed.identityGeneration) === hub.identity.identityGeneration && typeof signed.hubSignature === 'string' && crypto.verify(null, Buffer.from(responseCanonical), publicKey, Buffer.from(String(signed.hubSignature), 'base64url'));
    if (!signatureValid) {
