import { FOUNDATION_MIGRATION, FOUNDATION_MODEL_COUNT, isRuntimeTargetValid, safeBuildId } from '@/lib/foundation';
import { prisma } from '@/lib/prisma';

export const dynamic = 'force-dynamic';
export async function GET() {
  try {
    if (!isRuntimeTargetValid()) {
    ...
    const ready = migration[0]?.migration_name === FOUNDATION_MIGRATION
      && migration[0]?.checksum === expectedMigrationChecksum
      && Number(modelCount[0]?.count ?? 0) === FOUNDATION_MODEL_COUNT;
