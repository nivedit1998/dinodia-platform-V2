// Architecture: API boundary /public/rooms/requests/reject; validates a request and delegates to the platform domain/integration layers. Treat authentication, identifiers and response shapes as contracts shared with applicable web, iOS, Alexa, Hub Agent and support consumers.
...
export async function GET(req: NextRequest) {
  ...
  const url = new URL('/rooms/requests/reject', req.url);
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await approveOrRejectRoomAccessByToken({ tokenRaw: token, kind: RoomAccessApprovalKind.REJECT });
