...
export async function POST(request: Request, context: { params: Promise<{ ticketId: string; requestId: string }> }) {
  try {
    ...
    const code = `DNO-S-${randomSecret(18)}`;
    ...
    await prisma.$transaction(async (tx) => {
      const updated = await tx.supportAccessRequest.updateMany({ where: { id: row.id, requestedByEmployeeId: employee.id, status: 'APPROVED', codeHash: null }, data: { status: 'ISSUED', codeHash: sha256(code), codeIssuedAt: issuedAt, codeExpiresAt: expiresAt, employeeHandoffHash: sha256(employeeGrant), employeeHandoffEnvelope: employeeHandoffEnvelope, employeeHandoffIssuedAt: issuedAt, employeeHandoffExpiresAt: expiresAt } });
    ...
    return NextResponse.json({ ok: true, requestId: row.id, status: 'ISSUED', code, supportUrl: `${hub.cloudUrl.replace(/\/$/, '')}/support-access`, codeExpiresAt: expiresAt }, { headers: { 'Cache-Control': 'no-store', Pragma: 'no-cache' } });
